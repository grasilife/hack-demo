...
function arrayToTree(array = [], options = { id: "id", pid: "pid", children: "children" }) {
  ...
  const root_pid = [0, "0", undefined, "undefined", null, "null", "00000000-0000-0000-0000-000000000000", ""]; // 可能存在的根节点pid形式
  array.forEach(item => {
    // 筛选可以插入当前节点的所有子节点
    const children_array = array.filter(it => it[options.pid] === item[options.id]);
    if (item[options.children] && Array.isArray(item[options.children]) && item[options.children].length > 0) {
      // 去重合并数组
...
function patchTreeChain(
  ...
  const _out_put_data = []; // 声明一个导出数据盒子
  let _all_lack_data = []; // 声明一个全部需要补全的节点盒子
  data.forEach(i => {
    // 当一个节点在整个已选节点里找不到父节点时，并且此节点不是根节点时，从源数据中补全
    if (!data.find(t => t[options.Id] === i[options.ParentId]) && i[options.ParentId] !== options.root) {
      // 首先将记录在节点身上的父级树链拆分
      ...
      const _lack_parents = _parents.filter(e => data.findIndex(m => m[options.IdentityId] == e) === -1);
      // 合并全部需要补全的数据
...
function locationAfterDelete(data, delId, actId, useTree = false) {
  if (data.length === 1) {
    const _item = data.Parent ? data.Parent : { Id: useTree ? data[0].ParentId : "" };
    return { item: _item, after_data: [] };
  }
  const after_data = data.filter(item => item.Id !== delId);
  if (actId && delId !== actId) {
    return { item: null, after_data };
  }
  const cur_i = data.findIndex(item => item.Id === delId);
  const prv_item = cur_i > 0 ? data[cur_i - 1] : null;
  const next_item = cur_i !== data.length - 1 ? data[cur_i + 1] : null;
...
function splicParentsUntil(
  ...
  const coordinate_item = data.find(i => i[options.Id] === coordinate[options.CoordinateId]);
...
function intersectionBy(array1 = [], array2 = [], match = "Id") {
  ...
  const data = [];
  array2.forEach(item => {
    const match_success = array1.find(it => it[match] === item);
